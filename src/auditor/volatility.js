/**
 * Pure macro-volatility classification for the Cache Auditor. No SillyBunny
 * imports — directly testable under `node --test`.
 *
 * Classes, from worst to harmless:
 *  - random:   different output on every single evaluation
 *  - time:     changes with the clock
 *  - chat:     changes as the chat moves (last message, counts, activated lore)
 *  - state:    variable reads/writes — value drift and side effects
 *  - periodic: changes only on coarse boundaries (cache-friendly by design)
 *  - stable:   deterministic (cache-safe)
 */
